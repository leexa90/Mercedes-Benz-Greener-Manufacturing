...
def xgb_r2_score(preds, dtrain):
    ...
    return 'r2', r2_score(10**labels, 10**preds)+100
    ...
    for child in [5,20]:
        ...
        for num in range(0,5):
            ...
            for validation in range(0,max_n):
                ...
                train=train.set_value(dcv.index,name+str(depth)+'_'+str(num),10**model.predict(xgcv))
                test.set_value(test.index,name+str(depth)+'_'+str(num),test[name+str(depth)+'_'+str(num)]+10**model.predict(xgtest)/max_n)
                dictt[depth] += [a['cv']['r2'][model.best_iteration],]
            print '\n'
            print r2_score(10**train.y,train[name+str(depth)+'_'+str(num)])
for i in dictt:
