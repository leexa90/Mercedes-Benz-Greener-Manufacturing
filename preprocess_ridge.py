...
for alpha in [10,20,30,40,50]:
    ...
    for repeat in range(0,10):
        ...
        for validation in range(0,max_n):
            ...
            dtest=test
            iter_n = 2000
            reg = linear_model.Ridge (alpha = alpha,max_iter=iter_n)
        ...
        if name+'_'+str(alpha) not in dictt:
            dictt[alpha]=[xgb_r2_score(train[name+'_'+str(alpha)+'_'+str(repeat)],train.y.values)[1],]
        else:
            dictt[alpha]+=[xgb_r2_score(train[name+'_'+str(alpha)+'_'+str(repeat)],train.y.values)[1],]
    pred = [x for x in train.keys() if len(x.split('_')) >=2 and str(alpha) == x.split('_')[1]]
    train[alpha]=map(lambda x :np.mean(x) , np.array(train[pred]))
    print xgb_r2_score(train[alpha],train.y.values)
...
for i in dictt:
   x += [i,]
   y += [dictt[i],]
plt.plot(np.log(np.array(x)),y,'ro');plt.savefig('ridge')
plt.show()
die
target='y'
params = {}
...
train[predictors].to_csv('train_ridge.csv',index=0)
predictors=[i for i in test.keys() if ('depth' in i) or i in ['ID','y']]
test[predictors].to_csv('test_ridge.csv',index=0)
'''
