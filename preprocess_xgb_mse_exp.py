...
def log105(number):
    return np.log(number)/np.log(1.05)
    
def xgb_r2_score(preds, dtrain):
    labels = dtrain.get_label()
    return 'r2', r2_score(log105(labels), log105(preds))+100
...
for depth in [3,4,5,6]:
    ...
    for num in range(0,5):
        ...
        for validation in range(0,max_n):
            ...
            train=train.set_value(dcv.index,name+str(depth)+'_'+str(num),log105(model.predict(xgcv)))
            test.set_value(test.index,name+str(depth)+'_'+str(num),test[name+str(depth)+'_'+str(num)]+log105(model.predict(xgtest))/max_n)
