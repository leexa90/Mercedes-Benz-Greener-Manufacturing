...
def model():
    ...
    # compile this model
    model.compile(loss='mean_absolute_error', # one may use 'mean_absolute_error' as alternative
                  optimizer='adam',
...
for tries in range(0,10):
    ...
    pred_train['predM'+str(tries)]=0
    pred_test['predM'+str(tries)]=0
    for validation in range(0,max_n):
        ...
        temp=pred_test.set_value(
            test.index,
            'predM'+str(tries),
            pred_test['predM'+str(tries)]+estimator.predict(X_te,verbose=0)/max_n
            )
    train=train.merge(pred_train,on='ID')
    test=test.merge(pred_test,on='ID')
    print xgb_r2_score(train['predM'+str(tries)],train.y)
train['ID']=train['ID'].astype(np.int32)
test['ID']=test['ID'].astype(np.int32)
predictors=[x for x in train.keys() if 'pred' in x]
train[predictors+['ID','y',]].to_csv('train_keras_M.csv',index=0)    
test[predictors+['ID',]].to_csv('test_keras_M.csv',index=0)
die
